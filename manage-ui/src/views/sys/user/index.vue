<template>
  <div class="app-container">
    <el-row :gutter="20" class="header">
      <el-col :span="7">
       <el-input placeholder="请输入用户名..." v-model="queryForm.query" clearable @keyup.enter="initUserList" ></el-input>
      </el-col>
    ...
    <el-table :data="tableData" stripe style="width: 100%" @selectionChange="handleSelectionChange">
      <el-table-column type="selection" width="55" />
      <el-table-column prop="avatar" label="头像" width="80" align="center">
        <template v-slot="scope">
          <el-avatar :src="getServerUrl()+'video/img/'+scope.row.avatar" :size="50"/>
        </template>
      </el-table-column>
      <el-table-column prop="username" label="用户名" width="100" align="center"/>
      <el-table-column prop="roleList" label="拥有角色" width="200" align="center">
        <template v-slot="scope">
          <el-tag size="small" type="warning" v-for="(item,index) in scope.row.roleList" :key="index">{{item.name}}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="action" label="操作" width="400" fixed="right" align="center">
        <template v-slot="scope" >
          ...
          <el-popconfirm title="您确定要对这个用户重置密码吗？" @confirm="handleResetPassword(scope.row.id)">
            <template #reference>
...
import Dialog from "@/views/sys/user/component/Dialog.vue";
import RoleDialog from "@/views/sys/user/component/RoleDialog.vue";
const tableData = ref([])
